...
W    = 240
PW   = 1024      # 见下：预览必须比选中框还宽，否则是放大不是缩小
...
def main():
    ...
    #
    # 宽度这一栏踩过一次：先烤成 480，比例对了、取景也对了，可选中框有六百多宽，
    # 480 的图要被放大 1.42 倍才盖得满；全图一到又变成缩小 0.44 倍 ——
    # 源像素的缩放差了三倍多，看着就是「猛地放大一下」。
    # 所以预览必须比选中框还宽。框最宽是 innerWidth*0.62，取 1024 够用，
    # 落到任何尺寸都是缩小，只有清晰度在变，没有放大那一下。

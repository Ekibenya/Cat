...
本卡的记法作为选项交过去：
  ...
  --len 120-420    实测重做过一次：五百四十六条里最短一百二十一字，五分位一百四十三，
                   中位一百七十七，平均二百一十五。下限一百二十仍旧能拦住真的半截条
                   （那种在六十字以下）。头一次只看了三百七十八条，那时最短是一百三十七。
...
# 무늬 이름. 색이 아니라 무늬만 적는 줄이 있어서 따로 둔다.
PAT = ('\u4e91\u7eb9|\u70df\u7070|\u864e\u6591|\u4e09\u8272|\u73b3\u7469|'
       '\u6591|\u7eb9|\u82b1|\u6742')
C = '\u9ed1|\u767d|\u7070|\u8910|\u91d1|\u94f6|\u7ea2|\u68d5|\u9752|\u84dd|\u9ec4'
NUM = '[0-9\u3007\u96f6\u4e00\u4e8c\u4e09\u56db\u4e94\u516d\u4e03\u516b\u4e5d\u5341\u767e]'
FIELD = [
    ('年龄',  '概要|来历', NUM + r'+\s*岁|岁数|年纪'),
    ('身高',  '外貌',      NUM + r'+\s*厘米'),
    ('体重',  '外貌',      NUM + r'+\s*公斤'),
    # 「毛通身是黑的」「毛色是少见的云纹」—— 说法不一，所以看的是「毛」跟颜色挨着，
    # 不是非得写「毛色」两个字。初版只认死写法，白判了好几个。
    # 무늬 이름만 쓰는 줄도 있다 —— 「毛是点斑纹」. 그래서 색만이 아니라 무늬도 센다.
    ('毛色',  '外貌',      '毛色|头发|发色|毛[^\u3002]{0,8}(' + C + '|' + PAT + ')|'
                          '(' + C + ')[^\u3002]{0,4}\u6bdb|' + PAT),
    # 形态型有时写成「宫廷礼法系」这样的「系」，不写「型」。
    # 사람 쪽은 「因为是人，既没有猫耳朵也没有尾巴」라고 쓴다. 「人类」라고는 안 쓴다.
    # 「按族类算是属人」「耳朵尾巴都没有」——都是在写品种，只是没照我猜的语序写。
    # 判过一次假的，这两种写法补进来：属人／族类，以及「耳朵…没有」这个倒过来的说法。
    ('品种',  '外貌',      '品种|型|系|形态|人类|是人|属人|族类|'
                          '没有耳朵|没有猫耳|不是猫|耳朵[^。]{0,8}没有|耳[^。]{0,6}都没有'),
    # 「衣着是一条腰带上挂着铜牌的样子」这一句本来被判成没写服装 —— 那是这一关自己看漏了。
    # 认的词放得比初版宽：说的是身上有什么，不是非得用「穿」这个字。
    ('服装',  '外貌',      '衣服|衣裳|衣着|穿|披|裹|袍|裙|甲|胄|护胫|靴|鞋|布|绑着|系着|'
                          '戴|腰带|带子|挂|挎|绳|袋|套|巾|帽|冠|赤身|光着|不着|一丝不挂|赤脚|光脚'),
    ('出生地', '概要|来历', '生于|生在|出生|出身|生下|生长|长大|土生'),
    # 生地与住处是同一处的人很多（事实卡上就写着同一个地名）。
    # 那时候「在某处生下并长大」这一句就把两样都说完了，再写一遍反倒不像话。
    # 「在望岬落了脚」——「落脚」中间插了一个「了」。这是韩语译回来常见的样子，
    # 判过一次假的，所以这里放宽成「落…脚」。
    ('居住地', '概要',      '住|待在|落.{0,2}脚|如今在|现在在|留在|窝在|守着|长大|生长|一直在|没离开'),
]
